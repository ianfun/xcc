struct LinkerRunner : public DiagnosticHelper {
    ...
    LinkerRunner(xcc_context &context, StringRef linkerPath, StringRef filePath, StringRef outputPath)
        : DiagnosticHelper{context}, linkerPath{linkerPath}, filePath{filePath}, outputPath{outputPath} { }
    void run() {
        bool failed = false;
        info = llvm::sys::ExecuteNoWait(linkerPath, {filePath, "-o", outputPath}, llvm::None, llvm::None, 0, &ErrMsg,
                                        &failed);
        if (failed)
