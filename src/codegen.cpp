...
LLVM_NODISCARD Type wrap(CType ty) {
    ...
    switch (ty->k) {
        ...
        case TYFUNCTION:
        {
            Type *buf = alloc.Allocate<Type>(ty->params.size());
            size_t i = 0;
    ...
    if (g)
        ...
        if (currentfunction) {
            ...
            if (last_line == target)
                return;
            last_line = target;
            B().CreateStore(llvm::ConstantInt::get(types[x32], target), lt_line);
        }
    }
}
