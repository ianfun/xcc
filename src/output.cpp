struct IRModuleOutputFileHelper: public DiagnosticHelper {
    ...
    IRModuleOutputFileHelper(xcc_context &context, llvm::Module *M, StringRef outputPath) : DiagnosticHelper{context}, M{M}, outputPath{outputPath}, EC{}, OS{outputPath, EC} {
        if (EC) {
...
#pragma pop_macro("error")
    ...
    void assembly(llvm::TargetMachine &M) {
        return _target(M, llvm::CGFT_AssemblyFile);
    }
    void objectfile(llvm::TargetMachine &M) {
        return _target(M, llvm::CGFT_ObjectFile);
    }
    void finalize() {
        OS.close();
    }
