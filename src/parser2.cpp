...
struct Parser : public DiagnosticHelper {
    ...
    CType gettagByName(IdentRef Name, enum CTypeKind expected, Location loc) {
        ...
        if (r) {
            ...
            insertStmt(SNEW(DeclStmt){.loc = loc, .decl_idx = sema.tags.putSym(Name, info), .decl_ty = result});
        }
        return result;
    }
    ...
    size_t putsymtype2(IdentRef Name, CType yt) {
        ...
        if (it) {
            ...
            idx = sema.typedefs.putSym(
                Name, Variable_Info{.ty = yt, .loc = loc, .tags = ASSIGNED}); // function never undefined
        }
        return idx;
    }
    // typedef, variable
    ...
    bool checkInteger(CType ty) { return ty->k == TYPRIM && !(ty->k & (TYVOID | TYDOUBLE | TYFLOAT)); }
    ...
    Expr primary_expression() {
        ...
        case TIdentifier: {
            if (l.want_expr)
                ...
                ty->tags |= TYLVALUE;
                //        bool want_lvalue = sema.want_var || is_assigment_op(l.tok.tok) || l.tok.tok == TAddAdd ||
                //        l.tok.tok == TSubSub; if (it->val && !want_lvalue)
                //          return wrap(ty, it->val, loc);
                if (it->val && it->ty->tags & TYCONST)
                    return wrap(ty, it->val, loc);
...
NEXT:
    ...
    Expr conditional_expression(Expr start) {
        ...
        if (!compatible(lhs->ty, rhs->ty))
            return type_error(getLoc(), "incompatible type for conditional-expression: the left is %T, the right is %T",
                              lhs->ty, rhs->ty),
                   nullptr;
        conv(lhs, rhs);
    ...
    Stmt function_body(const xvector<Param> params, xvector<size_t> &args, Location loc) {
        ...
        for (size_t i = 0; i < params.size(); ++i)
            args[i] =
                sema.typedefs.putSym(params[i].name, Variable_Info{.ty = params[i].ty, .loc = loc, .tags = ASSIGNED});
        {
