...
void tools::gcc::Common::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (getTriple().isOSDarwin()) {
    ...
    CmdArgs.push_back(
        Args.MakeArgString(getDefaultUniversalArchName()));
  }
  ...
  for (const auto &II : Inputs) {
    ...
    if (types::isLLVMIR(II.getType()))
      ...
      D.Diag(diag::err_drv_no_ast_support) << getTripleString();
  ...
  const char *Exec = Args.MakeArgString(GetProgramPath(GCCName));
...
void tools::gnutools::StaticLibTool::ConstructJob(
  ...
  const char *Exec = Args.MakeArgString(GetStaticLibToolPath());
  C.addCommand(std::make_unique<Command>(JA, *this,
                                         ResponseFileSupport::AtFileCurCP(),
                                         Exec, CmdArgs, Inputs, Output));
}
...
void tools::gnutools::Linker::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (!Args.hasArg(options::OPT_nostdlib, options::OPT_r)) {
    if (!Args.hasArg(options::OPT_nodefaultlibs)) {
      ...
      if (getTriple().getArch() == llvm::Triple::sparc) {
...
void tools::gnutools::Assembler::ConstructJob(Compilation &C,
  ...
  case llvm::Triple::riscv32:
  case llvm::Triple::riscv64: {
    StringRef ABIName = riscv::getRISCVABI(Args, getTriple());
    CmdArgs.push_back("-mabi");
    CmdArgs.push_back(ABIName.data());
    StringRef MArchName = riscv::getRISCVArch(Args, getTriple());
  ...
  case llvm::Triple::sparcel: {
    CmdArgs.push_back("-32");
    std::string CPU = getCPUName(D, Args, getTriple());
    CmdArgs.push_back(
        sparc::getSparcAsmModeForCPU(CPU, getTriple()));
  ...
  case llvm::Triple::thumb:
  case llvm::Triple::thumbeb: {
    const llvm::Triple &Triple2 = getTriple();
  ...
  case llvm::Triple::aarch64_be: {
    CmdArgs.push_back(
        getArch() == llvm::Triple::aarch64_be ? "-EB" : "-EL");
    Args.AddLastArg(CmdArgs, options::OPT_march_EQ);
    normalizeCPUNamesForAssembler(Args, CmdArgs);
  ...
  case llvm::Triple::mips64el: {
    StringRef CPUName;
    StringRef ABIName;
    mips::getMipsCPUAndABI(Args, getTriple(), CPUName, ABIName);
    ...
    if (Arg *A = Args.getLastArg(options::OPT_mfp32, options::OPT_mfpxx,
      ...
      A->render(Args, CmdArgs);
                   ...
                   Args, getTriple(), CPUName, ABIName,
                   mips::getMipsFloatABI(getDriver(), Args,
                                         getTriple())))
  ...
  const char *Exec =
      Args.MakeArgString(GetProgramPath(DefaultAssembler));
  C.addCommand(std::make_unique<Command>(JA, *this,
                                         ResponseFileSupport::AtFileCurCP(),
                                         Exec, CmdArgs, Inputs, Output));
