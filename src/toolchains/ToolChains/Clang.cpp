...
void Clang::AddPreprocessingOptions(Compilation &C, const JobAction &JA,
  ...
  const bool IsIAMCU = getTriple().isOSIAMCU();
  ...
  if (JA.isOffloading(Action::OFK_Cuda))
    AddCudaIncludeArgs(Args, CmdArgs);
  if (JA.isOffloading(Action::OFK_HIP))
    AddHIPIncludeArgs(Args, CmdArgs);
  ...
  if (!IsIAMCU)
    ...
    AddIAMCUIncludeArgs(Args, CmdArgs);
  }
...
void Clang::AddARMTargetArgs(const llvm::Triple &Triple, const ArgList &Args,
  ...
  RenderARMABI(getDriver(), Triple, Args, CmdArgs);
...
void Clang::AddAArch64TargetArgs(const ArgList &Args,
                                 ArgStringList &CmdArgs) const {
  const llvm::Triple &Triple = getEffectiveTriple();
...
void Clang::AddLoongArchTargetArgs(const ArgList &Args,
  ...
  CmdArgs.push_back("-target-abi");
  CmdArgs.push_back(
      loongarch::getLoongArchABI(Args, getTriple()).data());
}
...
void Clang::AddPPCTargetArgs(const ArgList &Args,
  ...
  bool IEEELongDouble = defaultToIEEELongDouble();
  ...
  ppc::FloatABI FloatABI =
      ppc::getPPCFloatABI(getDriver(), Args);
...
void Clang::AddSparcTargetArgs(const ArgList &Args,
  ...
  sparc::FloatABI FloatABI =
      sparc::getSparcFloatABI(getDriver(), Args);
...
void Clang::AddSystemZTargetArgs(const ArgList &Args,
  ...
  systemz::FloatABI FloatABI =
      systemz::getSystemZFloatABI(getDriver(), Args);
...
void Clang::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  // device mode (i.e., getTriple() is NVPTX/AMDGCN, not
  ...
  // remarks.
  bool IsFullySupported = getTriple().isOSDarwin() &&
...
ObjCRuntime Clang::AddObjCRuntimeArgs(const ArgList &args,
  ...
  if (runtimeArg &&
    ...
    if (runtime.tryParse(value)) {
      getDriver().Diag(diag::err_drv_unknown_objc_runtime)
          << value;
    }
  ...
  if (Arg *abiArg = args.getLastArg(options::OPT_fobjc_abi_version_EQ)) {
    ...
    if (value == "1")
      ...
      getDriver().Diag(diag::err_drv_clang_unsupported) << value;
  } else {
...
// Determine the non-fragile ABI version to use.
  ...
  if (!runtimeArg) {
    switch (rewriteKind) {
    case RK_None:
      runtime = getDefaultObjCRuntime(isNonFragile);
      break;
    case RK_Fragile:
...
void Clang::AddClangCLArgs(const ArgList &Args, types::ID InputType,
  ...
  bool isNVPTX = getTriple().isNVPTX();
...
void ClangAs::AddX86TargetArgs(const ArgList &Args,
                               ArgStringList &CmdArgs) const {
  addX86AlignBranchArgs(getDriver(), Args, CmdArgs,
                        /*IsLTO=*/false);
...
void ClangAs::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  addClangCC1ASTargetOptions(Args, CmdArgs);
  ...
  if (SourceAction->getType() == types::TY_Asm ||
    ...
    addDebugPrefixMapArg(getDriver(), getToolChain(), Args,
                         CmdArgs);
  ...
  // analysis.
  if (UseDwarfDebugFlags()) {
    ArgStringList OriginalArgs;
    for (const auto &Arg : Args)
      Arg->render(Args, OriginalArgs);

    SmallString<256> Flags;
    const char *Exec = getDriver().getClangProgramPath();
    EscapeSpacesAndBackslashes(Exec, Flags);
    for (const char *OriginalArg : OriginalArgs) {
      SmallString<128> EscapedArg;
      EscapeSpacesAndBackslashes(OriginalArg, EscapedArg);
...
void OffloadBundler::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  C.addCommand(std::make_unique<Command>(
      ...
      TCArgs.MakeArgString(GetProgramPath(getShortName())),
      CmdArgs, None, Output));
}
...
void OffloadPackager::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  C.addCommand(std::make_unique<Command>(
      JA, *this, ResponseFileSupport::None(),
      Args.MakeArgString(GetProgramPath(getShortName())),
      CmdArgs, Inputs, Output));
}
...
void LinkerWrapper::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  const char *Exec =
      Args.MakeArgString(GetProgramPath("clang-linker-wrapper"));
