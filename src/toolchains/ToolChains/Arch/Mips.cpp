...
void getMipsCPUAndABI(const ArgList &Args, const llvm::Triple &Triple,
...
std::string getMipsABILibSuffix(const ArgList &Args,
...
// and -mfloat-abi=.
enum FloatABI getMipsFloatABI(const Driver &D, const ArgList &Args,
                                     const llvm::Triple &Triple) {
  ...
  if (Arg *A =
    ...
    if (A->getOption().matches(options::OPT_msoft_float))
      ...
      if (ABI == FloatABI::Invalid && !StringRef(A->getValue()).empty()) {
        printf("clang::diag::err_drv_invalid_mfloat_abi) << A->getAsString(Args)");
        ABI = FloatABI::Hard;
      }
    }
  }
...
void getMIPSTargetFeatures(const Driver &D, const llvm::Triple &Triple,
  ...
  enum FloatABI FloatABI = getMipsFloatABI(D, Args, Triple);
  ...
  if (Arg *A = Args.getLastArg(options::OPT_mnan_EQ)) {
    ...
    if (Val == "2008") {
      if (getIEEE754Standard(CPUName) & Std2008)
      ...
      if (getIEEE754Standard(CPUName) & Legacy)
        Features.push_back("-nan2008");
      else {
        Features.push_back("+nan2008");
  ...
  if (Arg *A = Args.getLastArg(options::OPT_mfp32, options::OPT_mfpxx,
    ...
    if (A->getOption().matches(options::OPT_mfp32))
      ...
      Features.push_back("+fp64");
  } else if (shouldUseFPXX(Args, Triple, CPUName, ABIName, FloatABI)) {
    Features.push_back("+fpxx");
    Features.push_back("+nooddspreg");
  } else if (isFP64ADefault(Triple, CPUName)) {
    Features.push_back("+fp64");
    Features.push_back("+nooddspreg");
  }
  ...
  if (Arg *A = Args.getLastArg(options::OPT_mindirect_jump_EQ)) {
    ...
    if (Val == "hazard") {
      ...
      if (B && B->getOption().matches(options::OPT_mmicromips))
        ...
        D.Diag(diag::err_drv_unsupported_indirect_jump_opt)
            << "hazard" << "mips16";
      else if (supportsIndirectJumpHazardBarrier(CPUName))
...
bool hasMipsAbiArg(const ArgList &Args, const char *Value) {
  Arg *A = Args.getLastArg(options::OPT_mabi_EQ);
  return A && (A->getValue() == StringRef(Value));
}

bool isUCLibc(const ArgList &Args) {
  Arg *A = Args.getLastArg(options::OPT_m_libc_Group);
  return A && A->getOption().matches(options::OPT_muclibc);
}

bool isNaN2008(const Driver &D, const ArgList &Args,
                     const llvm::Triple &Triple) {
...
bool isFPXXDefault(const llvm::Triple &Triple, StringRef CPUName,
                         StringRef ABIName, enum FloatABI FloatABI) {
  if (Triple.getVendor() != llvm::Triple::ImaginationTechnologies &&
      Triple.getVendor() != llvm::Triple::MipsTechnologies &&
      !Triple.isAndroid())
    return false;
...
bool shouldUseFPXX(const ArgList &Args, const llvm::Triple &Triple,
                         StringRef CPUName, StringRef ABIName,
                         enum FloatABI FloatABI) {
