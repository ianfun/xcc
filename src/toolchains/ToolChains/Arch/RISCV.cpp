...
void getRISCVTargetFeatures(const Driver &D, const llvm::Triple &Triple,
  ...
  if (Arg *A = Args.getLastArg(options::OPT_mcpu_EQ)) {
    ...
    if (!getRISCFeaturesFromMcpu(Triple, CPU, Features))
      printf("clang::diag::err_drv_unsupported_option_argument) << A->getSpelling() << CPU");
  }
  ...
  if (Args.hasFlag(options::OPT_mrelax, options::OPT_mno_relax, true)) {
    ...
    if (getDebugFissionKind(D, Args, A) != DwarfFissionKind::None)
      printf("clang::diag::err_drv_riscv_unsupported_with_linker_relaxation)<< A->getAsString(Args)");
  } else {
    Features.push_back("-relax");
  }
...
StringRef getRISCVABI(const ArgList &Args, const llvm::Triple &Triple) {
