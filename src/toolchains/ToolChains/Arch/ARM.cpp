...
static void checkARMArchName(const Driver &D, const Arg *A, const ArgList &Args,
  ...
  std::string MArch = getARMArch(ArchName, Triple);
...
static void checkARMCPUName(const Driver &D, const Arg *A, const ArgList &Args,
  ...
  std::string CPU = getARMTargetCPU(CPUName, ArchName, Triple);
  llvm::ARM::ArchKind ArchKind =
    getLLVMArchKindForARM(CPU, ArchName, Triple);
  if (ArchKind == llvm::ARM::ArchKind::INVALID ||
      (Split.second.size() &&
...
enum ReadTPMode getReadTPMode(const Driver &D, const ArgList &Args,
                                   const llvm::Triple &Triple, bool ForAS) {
  if (Arg *A = Args.getLastArg(options::OPT_mtp_mode_EQ)) {
    enum ReadTPMode ThreadPointer =
        llvm::StringSwitch<enum ReadTPMode>(A->getValue())
...
void setArchNameInTriple(const Driver &D, const ArgList &Args,
  ...
  std::string CPU = Triple.isOSBinFormatMachO()
                        ? :getARMCPUForMArch(MArch, Triple).str()
                        : :getARMTargetCPU(MCPU, MArch, Triple);
  StringRef Suffix = getLLVMArchSuffixForARM(CPU, MArch, Triple);
  ...
  if (IsMProfile && ARMModeRequested) {
    if (MCPU.size())
      printf("D.Diag(diag::err_cpu_unsupported_isa) << CPU << \"ARM\"");
    else
      printf("D.Diag(diag::err_arch_unsupported_isa) << getARMArch(MArch, Triple) << \"ARM\"");
  }
  ...
  if (InputType != types::TY_PP_Asm)
    ...
    if (WaMCPU.size() || WaMArch.size()) {
      ...
      Suffix = getLLVMArchSuffixForARM(WaMCPU, WaMArch, Triple);
    }
  }
...
void setFloatABIInTriple(const Driver &D, const ArgList &Args,
                              llvm::Triple &Triple) {
  bool isHardFloat =
      (getARMFloatABI(D, Triple, Args) == FloatABI::Hard);
  ...
  case llvm::Triple::MuslEABIHF:
    ...
    break;
  default: {
    enum FloatABI DefaultABI = getDefaultFloatABI(Triple);
    if (DefaultABI != FloatABI::Invalid &&
        isHardFloat != (DefaultABI == FloatABI::Hard)) {
      Arg *ABIArg =
...
enum FloatABI getARMFloatABI(const ToolChain &TC, const ArgList &Args) {
  return getARMFloatABI(TC.getDriver(), TC.getEffectiveTriple(), Args);
}

enum FloatABI getDefaultFloatABI(const llvm::Triple &Triple) {
...
// -mfloat-abi=.
enum FloatABI getARMFloatABI(const Driver &D, const llvm::Triple &Triple,
                                  const ArgList &Args) {
  ...
  if (Arg *A =
    ...
    if (A->getOption().matches(options::OPT_msoft_float)) {
      ...
      ABI = llvm::StringSwitch<enum FloatABI>(A->getValue())
...
void getARMTargetFeatures(const Driver &D, const llvm::Triple &Triple,
  ...
  FloatABI ABI = getARMFloatABI(D, Triple, Args);
...
fp16_fml_fallthrough:
  ...
  if (HasSHA2 || HasAES) {
    StringRef ArchSuffix = getLLVMArchSuffixForARM(
        getARMTargetCPU(CPUName, ArchName, Triple), ArchName, Triple);
...
std::string getARMArch(StringRef Arch, const llvm::Triple &Triple) {
  std::string MArch;
  if (!Arch.empty())
...
llvm::ARM::ArchKind getLLVMArchKindForARM(StringRef CPU, StringRef Arch,
  ...
  if (CPU == "generic" || CPU.empty()) {
    std::string ARMArch = getARMArch(Arch, Triple);
...
void appendBE8LinkFlag(const ArgList &Args, ArgStringList &CmdArgs,
                            const llvm::Triple &Triple) {
