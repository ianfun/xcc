...
static bool DecodeAArch64Features(const Driver &D, StringRef text,
  ...
  for (StringRef Feature : Split) {
    ...
    if (!FeatureName.empty())
      ...
      printf("clang::diag::err_drv_no_neon_modifier");
    else
      return false;
...
void aarch64::getAArch64TargetFeatures(const Driver &D,
  ...
  if (!success) {
    ...
    if (!WaMArch.empty())
      printf("diag::err_drv_unsupported_option_argument \"-march=\" << WaMArch");
    else
      printf("diag::err_drv_unsupported_option_argument");
  ...
  if (Arg *A = Args.getLastArg(options::OPT_mtp_mode_EQ)) {
    ...
    if (Mtp == "el3")
      ...
      printf("diag::err_drv_invalid_mtp) << A->getAsString(Args)");
  }
