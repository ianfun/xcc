...
void darwin::Linker::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  if (Args.hasArg(options::OPT_ccc_arcmt_check,
    ...
    for (const auto &Arg : Args)
      Arg->claim();
    const char *Exec =
        Args.MakeArgString(GetProgramPath("touch"));
    CmdArgs.push_back(Output.getFilename());
  ...
  bool LinkerIsLLD;
  const char *Exec =
      Args.MakeArgString(GetLinkerPath(&LinkerIsLLD));
  ...
  if (willEmitRemarks(Args) &&
      checkRemarksOptions(getDriver(), Args,
                          getTriple()))
    renderRemarksOptions(Args, CmdArgs, getTriple(), Output, JA);
  ...
  SmallString<128> StatsFile =
      getStatsFileName(Args, Output, Inputs[0], getDriver());
  if (!StatsFile.empty()) {
    CmdArgs.push_back("-mllvm");
  ...
  StringRef Parallelism = getLTOParallelism(Args, getDriver());
...
void darwin::Lipo::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  const char *Exec = Args.MakeArgString(GetProgramPath("lipo"));
  C.addCommand(std::make_unique<Command>(JA, *this, ResponseFileSupport::None(),
                                         Exec, CmdArgs, Inputs, Output));
}
...
void darwin::Dsymutil::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  const char *Exec =
      Args.MakeArgString(GetProgramPath("dsymutil"));
  C.addCommand(std::make_unique<Command>(JA, *this, ResponseFileSupport::None(),
                                         Exec, CmdArgs, Inputs, Output));
}
...
void darwin::VerifyDebug::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  const char *Exec =
      Args.MakeArgString(GetProgramPath("dwarfdump"));
  C.addCommand(std::make_unique<Command>(JA, *this, ResponseFileSupport::None(),
                                         Exec, CmdArgs, Inputs, Output));
}
