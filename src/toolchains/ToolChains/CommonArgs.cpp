...
void tools::AddLinkerInputs(Command &C, const ArgList &args) {
  for (Arg *A : args) {
...
void tools::addLinkerCompressDebugSectionsOption(
  ...
  if (const Arg *A = Args.getLastArg(options::OPT_gz_EQ)) {
    ...
    if (V == "none" || V == "zlib" || V == "zstd")
      ...
      printf("diag::err_drv_unsupported_option_argument) << A->getSpelling() << V");
  }
}
...
bool tools::GetSDLFromOffloadArchive(
  ...
  DeviceTriple += '-';
  std::string NormalizedTriple = T.getTriple().normalize();
  ...
  const char *UBProgram = DriverArgs.MakeArgString(
      T.GetProgramPath("clang-offload-bundler"));
