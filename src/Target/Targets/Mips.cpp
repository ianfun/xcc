...
static constexpr llvm::StringLiteral ValidCPUNames2[] = {
...
bool MipsTargetInfo::isValidCPUName(StringRef Name) const {
  return llvm::is_contained(ValidCPUNames2, Name);
}

void MipsTargetInfo::fillValidCPUList(
    SmallVectorImpl<StringRef> &Values) const {
  Values.append(std::begin(ValidCPUNames2), std::end(ValidCPUNames2));
}
...
ArrayRef<Builtin::Info> MipsTargetInfo::getTargetBuiltins() const {
  return llvm::makeArrayRef(BuiltinInfo, Mips::LastTSBuiltin -
...
bool MipsTargetInfo::validateTarget() const {
  // microMIPS64R6 backend was removed.
  if (getTriple().isMIPS64() && IsMicromips && (ABI == "n32" || ABI == "n64")) {
    printf("(diag::err_target_unsupported_cpu_for_micromips) << CPU");
    return false;
  }
  // FIXME: It's valid to use O32 on a 64-bit CPU but the backend can't handle
  //        this yet. It's better to fail here than on the backend assertion.
  if (processorSupportsGPR64() && ABI == "o32") {
    printf("diag::err_target_unsupported_abi) << ABI << CPU");
    return false;
  }
  ...
  if (getTriple().isMIPS64() && ABI == "o32") {
    printf("diag::err_target_unsupported_abi_for_triple) << ABI << getTriple().str()");
    return false;
  }
  ...
  if (FPMode == FPXX && (ABI == "n32" || ABI == "n64")) {
    printf("diag::err_unsupported_abi_for_opt) << -mfpxx << o32");
    return false;
  }
