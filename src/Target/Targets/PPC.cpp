...
// set of options.
static bool ppcUserFeaturesCheck(const std::vector<std::string> &FeaturesVec) {
...
bool PPCTargetInfo::initFeatureMap(
  ...
  if (CPU == "pwr10" || CPU == "power10") {
    initFeatureMap(Features, "pwr9", FeaturesVec);
  ...
  if (CPU == "future") {
    initFeatureMap(Features, "pwr10", FeaturesVec);
    addFutureSpecificFeatures(Features);
  }

  if (!ppcUserFeaturesCheck(FeaturesVec))
    return false;
...
static constexpr llvm::StringLiteral ValidCPUNames5[] = {
...
bool PPCTargetInfo::isValidCPUName(StringRef Name) const {
  return llvm::is_contained(ValidCPUNames5, Name);
}

void PPCTargetInfo::fillValidCPUList(SmallVectorImpl<StringRef> &Values) const {
  Values.append(std::begin(ValidCPUNames5), std::end(ValidCPUNames5));
}
...
ArrayRef<Builtin::Info> PPCTargetInfo::getTargetBuiltins() const {
  return llvm::makeArrayRef(BuiltinInfo, PPC::LastTSBuiltin -
