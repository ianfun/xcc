...
bool X86TargetInfo::initFeatureMap(
  ...
  if (!TargetInfo::initFeatureMap(Features, CPU, UpdatedFeaturesVec))
    return false;
...
bool X86TargetInfo::handleTargetFeatures(std::vector<std::string> &Features) {
  ...
  if ((FPMath == FP_SSE && SSELevel < SSE1) ||
    ...
    printf("Diags.Report(diag::err_target_unsupported_fpmath) << (FPMath == FP_SSE ? \"sse\" :\"387\")");
    return false;
  }
...
ArrayRef<Builtin::Info> X86_32TargetInfo::getTargetBuiltins() const {
  return llvm::makeArrayRef(BuiltinInfoX86, X86::LastX86CommonBuiltin -
