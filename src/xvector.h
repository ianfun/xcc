...
struct xvector {
	...
	LLVM_NODISCARD size_t size_in_bytes() const {
		return p->_length * sizeof(_xvector_impl) * sizeof(T);
	}
	LLVM_NODISCARD size_t capacity() const {
		return p->_capacity;
	}
	LLVM_NODISCARD const T *data() const {
		return p->_data;
	}
	LLVM_NODISCARD T *data() {
		return p->_data;
	}
	...
	void push_back(const T &c) {
		if (p->_length >= p->_capacity) {
			p->_capacity *= 3;
			p->_capacity /= 2;
			p = reinterpret_cast<p_xvector_impl>(llvm::safe_realloc(p, sizeof(_xvector_impl) + p->_capacity * sizeof(T)));
		}
		p->_data[p->_length++] = c;
	}
	void push_back(T &&c) {
	...
	LLVM_NODISCARD T front() const {
		return *p->_data;
	}
	LLVM_NODISCARD T &front() {
		return *p->_data;
	}
	LLVM_NODISCARD T back() const {
		return p->_data[p->_length - 1];
	}
	LLVM_NODISCARD T &back() {
		return p->_data[p->_length - 1];
	}
	LLVM_NODISCARD const T *begin() const {
		return p->_data;
	};
	LLVM_NODISCARD T *begin() {
		return p->_data;
	}
	LLVM_NODISCARD const T *end() const {
		return p->_data + p->_length;
	}
	LLVM_NODISCARD T *end() {
		return p->_data + p->_length;
	}
};
